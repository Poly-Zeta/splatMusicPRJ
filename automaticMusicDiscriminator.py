...
blankLogtxt,baseImg=widgetWindow.print4imgBlank("GPU SETUP",blankLogtxt,baseImg)
...
blankLogtxt,baseImg=widgetWindow.print4imgBlank("SETUP",blankLogtxt,baseImg)
#曲名のラベル
...
samplingCounter=0
blankLogtxt,baseImg=widgetWindow.print4imgBlank("OK",blankLogtxt,baseImg)
blankLogtxt,baseImg=widgetWindow.print4imgBlank("AUDIO SETUP",blankLogtxt,baseImg)
#録音デバイスの設定
...
blankLogtxt,baseImg=widgetWindow.print4imgBlank("CAM SETUP",blankLogtxt,baseImg)
#カメラ設定
...
blankLogtxt,baseImg=widgetWindow.print4imgBlank("READY",blankLogtxt,baseImg)
blankLogtxt,baseImg=widgetWindow.print4imgBlank("SEARCHING 'GO!'",blankLogtxt,baseImg)
blankLogtxt,baseImg=widgetWindow.print4imgBlank("MANUAL START:S",blankLogtxt,baseImg)
blankLogtxt,baseImg=widgetWindow.print4imgBlank("MANUAL ITRPT:Ctrl+C",blankLogtxt,baseImg)
...
while 1:
    try:
        ...
        # if((np.count_nonzero(gray==img_start)>285000) or (cv2.waitKey(1) & 0xFF == ord('s'))):
            ...
            stream.close()
            p.terminate()
            blankLogtxt,baseImg=widgetWindow.print4imgBlank("DONE",blankLogtxt,baseImg)
            ...
            blankLogtxt,baseImg=widgetWindow.print4imgBlank('LABEL:'+pred_label,blankLogtxt,baseImg)
            blankLogtxt,baseImg=widgetWindow.print4imgBlank('SCORE:'+str(score),blankLogtxt,baseImg)
            ...
            #     frames_per_buffer=CHUNK
            # )
            blankLogtxt,baseImg=widgetWindow.print4imgBlank("SEARCHING 'FINISH'",blankLogtxt,baseImg)
            blankLogtxt,baseImg=widgetWindow.print4imgBlank("MANUAL END:E",blankLogtxt,baseImg)
            print("Press e Key...")
            while 1:
                ret,frame = cap.read()
                frame = cv2.resize(frame,(width, height))
                ...
                if ((compareGrey>267000)or(cv2.waitKey(1) & 0xFF == ord('e'))):
                    blankLogtxt,baseImg=widgetWindow.print4imgBlank("GAME END",blankLogtxt,baseImg)
                    print("game end")
                    break
            # cv2.destroyAllWindows()
            # cv2.imshow('frame',img_end)
            ...
            for i in range(15):
                time.sleep(1)
                print("waitnow "+str(i))
            buttleResult,buttleRule=urlKnocker.getNewestButtleResult(urlKnocker.getJson("https://app.splatoon2.nintendo.net/api/results/"\
                    +str(urlKnocker.getNewestButtleNumber(urlKnocker.getJson("https://app.splatoon2.nintendo.net/api/results"\
            )))))
            baseImg=widgetWindow.printButtleResult(buttleResult,buttleRule,baseImg)
            print("next")
